 ...
 * generally broadcast from every vehicle at a nominal rate of 10 Hz.//10 times in a sec
 */
...
public:
	 ...
	 * \return none
	 */
	void Setup(Ipv4InterfaceContainer & i, int nodeId, Time totalTime,
	...
	static int pruneTime;

	//neighbor table management function
	void PrintNeighborTable();
	void AddInNeighborTable(uint16_t neighbor);
	void DeleteInNeighborTable();
	void PruneNeighborTable();
...
private:
	 ...
	 * \return none
	 */
	void HandleReceivedBsmPacket(Ptr<Packet> packet, Ptr<Node> txNode, Ptr<Node> rxNode);
	...
	//variable to maintain neighbor table
	Ptr<Neighbortable> m_neighbor_table_header;
	Ptr<Neighbortable> m_neighbor_table_trailer;
};
